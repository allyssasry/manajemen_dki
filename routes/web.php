...
/* =======================
   GANTI: supervisor → kd
   ======================= */
Route::prefix('kd')->name('kd.')->middleware('auth')->group(function () {
    Route::get('/notifications', [SupervisorController::class, 'notifications'])->name('notifications');
...
// Middleware inline (tanpa Kernel): pastikan role = kd (Kepala Divisi)
/* =======================
   GANTI: supervisor → kd
   ======================= */
Route::prefix('kd')
    ...
    ->group(function () {
        ...
        Route::get('/projects/{project}', [SupervisorController::class, 'show'])
            ->name('projects.show');   // ← cukup "projects.show", karena sudah ada prefix 'kd.'
    });


/* =======================
   GANTI: /supervisor/dashboard → /kd/dashboard
          name supervisor.dashboard → kd.dashboard
   ======================= */
Route::middleware(['auth'])->group(function () {
    Route::get('/kd/dashboard', [SupervisorDashboardController::class, 'index'])
        ->name('kd.dashboard');
});
...
Route::get('/', function () {
    // Kalau sudah login, boleh sekalian diarahkan ke dashboard sesuai role
    if (Auth::check()) {
        $user = Auth::user();

        return match ($user->role) {
            'it'              => redirect()->route('it.dashboard'),
            'digital_banking' => redirect()->route('dig.dashboard'),
            'kd'              => redirect()->route('kd.dashboard'),
            default           => redirect()->route('login'),
        };
    }

    // Kalau belum login, arahkan ke form login
...
Route::post('/projects/{project}/attachments', [ProjectController::class, 'addAttachment'])
    ->name('projects.attachments.add');

// Hapus lampiran
Route::delete('/attachments/{attachment}', [ProjectController::class, 'deleteAttachment'])
    ->name('projects.attachments.delete');

Route::middleware(['auth'])->prefix('semua')->name('semua.')->group(function () {
    // list progress/proyek versi "semua"
    Route::get('/progresses', [ProjectController::class, 'progresses'])
        ->name('progresses');

    // form tambah project (halaman semua/projects/form.blade.php)
    ...
    // form edit project (pakai halaman yang sama)
