{{-- 
    Full Sidebar (Drawer) - Mobile & Desktop
...
<aside id="sidebar"
    ...
    <nav class="flex-1 overflow-y-auto py-3 text-sm font-medium text-gray-700">
        ...
        <a href="{{ route($progressRoute ?? 'semua.progresses') }}"
            class="flex items-center gap-3 px-5 py-2.5 rounded-xl
              {{ ($isProgressActive ?? false) ? 'bg-[#FFF2F2] text-[#7A1C1C] font-semibold' : 'hover:bg-[#FFF2F2] text-gray-800' }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 flex-none"
                fill="{{ ($isProgressActive ?? false) ? '#7A1C1C' : 'black' }}" viewBox="0 0 24 24">
        ...
        <a href="{{ route($notifRoute) }}"
            ...
            @if(($unreadNotifCount ?? 0) > 0)
                <span class="inline-flex items-center justify-center min-w-[20px] h-5 rounded-full bg-[#7A1C1C] text-white text-[11px] font-bold px-1.5">
