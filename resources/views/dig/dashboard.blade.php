...
<head>
    ...
    <script>
        (function() {
            try {
                var persisted = localStorage.getItem('dig.sidebar.open') === '1';
...
<aside id="sidebar"
    ...
    <div id="pageWrapper" class="md:ml-16">
            ...
            <div class="{{ $container }} py-3 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <button id="sidebarOpenBtn"
                        ...
                        <svg xmlns="http://www.w3.org/200/svg" class="w-5 h-5" viewBox="0 0 24 24"
                            fill="currentColor">
                            <path d="M3 3h18v2H3V3zm0 6h18v2H3V9zm0 6h18v2H3v-2z" />
                        </svg>
                    </button>
                    <span class="text-lg md:text-xl font-bold text-white select-none">Dashboard</span>
                </div>
                ...
                @php
                    ...
                    $fallbackSvg =
                        ...
                        rawurlencode(
                            '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64"><rect width="100%" height="100%" rx="12" ry="12" fill="#7A1C1C"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="Arial" font-size="28" fill="#fff">' .
                                $initial .
                                '</text></svg>',
                        );
                    ...
                    $avatarUrl = $rawUrl
                        ? $rawUrl . (str_contains($rawUrl, '?') ? '&' : '?') . 'ck=' . $extraKey
                        : $fallbackSvg;
                @endphp
        ...
        @php
            ...
            if (!isset($digitalUsers) || !($digitalUsers instanceof Collection)) {
                try {
                    $digitalUsers = \App\Models\User::where('role', 'digital_banking')
                        ->orderByRaw('COALESCE(NULLIF(name, \'\'), username) ASC')
            ...
            $mode = request('mode', 'real'); // real (realisasi update) | confirm (terkonfirmasi)
            ...
            $between = function ($date) use ($periodStart, $periodEnd) {
                ...
                try {
                    ...
                    return $d->betweenIncluded($periodStart, $periodEnd);
                } catch (\Throwable $e) {
                    return false;
                }
            };
            ...
            $projectTouchesPeriod = function ($project) use ($between, $overlapRange) {
                if ($between($project->completed_at ?? null)) {
                    return true;
                }
                if ($between($project->created_at ?? null)) {
                    return true;
                }
                foreach ($project->progresses ?? collect() as $pr) {
                    if ($overlapRange($pr->start_date ?? null, $pr->end_date ?? null)) {
                        return true;
                    }
                    foreach ($pr->updates ?? collect() as $up) {
            ...
            $mine = $all->filter(
                fn($p) => (int) ($p->digital_banking_id ?? 0) === $userId || (int) ($p->created_by ?? 0) === $userId,
            );
            ...
            $chartProject = $chartProjectId
                ? $displayProjects->firstWhere('id', $chartProjectId)
                : $displayProjects->first();
            $chartTitle = $chartProject ? $chartProject->name : '-';

            /**
             * ====== LOGIKA GRAFIK:
             * - Titik HANYA pada tanggal yg ADA UPDATE.
             * - Garis BERHENTI di tanggal stopReal (kandidat).
             */
            $chartPoints = [];
            if ($chartProject) {
                $candStart = collect();
                ...
                foreach ($chartProject->progresses as $pr) {
                    foreach ($pr->updates as $up) {
                        try {
                            ...
                            if ($ud->betweenIncluded($from, $to)) {
                                $updateDates->push($ud->copy()->startOfDay());
                            }
                        } catch (\Throwable $e) {
                        }
                    }
                ...
                if ($mode === 'confirm') {
                    $latestConfirm = collect($chartProject->progresses)
                        ->map(function ($pr) {
                    ...
                    foreach ($chartProject->progresses as $pr) {
                        $last = $pr->updates
                            ->filter(function ($u) use ($d) {
                                try {
                                    return Carbon::parse($u->update_date)->toDateString() <= $d->toDateString();
                                } catch (\Throwable $e) {
                                    return false;
                                }
                            })
                        ...
                        if ($mode === 'confirm' && is_null($pr->confirmed_at)) {
                            $val = 0;
                        }
                        $sum += $val;
                    }
        ...
        <section class="{{ $container }} mt-4">
            <div class="rounded-2xl border border-red-200 bg-white p-3 md:p-4">
                <div class="flex items-center justify-between">
                    ...
                    <form method="GET" action="{{ route('dig.dashboard') }}"
                        class="flex items-center gap-2 text-xs">
                        <input type="hidden" name="month_start" value="{{ $monthStartVal }}">
                        <input type="hidden" name="month_end" value="{{ $monthEndVal }}">
                        <select name="chart_project_id"
                            class="border rounded-md px-2 py-1 bg-[#FFFFFF] border-red-200">
                            @foreach ($displayProjects as $p)
                                <option value="{{ $p->id }}"
                                    {{ $chartProject && $chartProject->id === $p->id ? 'selected' : '' }}>
                                    {{ $p->name }}</option>
                            @endforeach
                        </select>
                        <select name="mode" class="border rounded-md px-2 py-1 bg-[#FFFFFF] border-red-200">
                            <option value="real" {{ $mode === 'real' ? 'selected' : '' }}>Realisasi (update)
                            </option>
                ...
                <div class="mt-3 rounded-xl border border-red-100 bg-[#FFF8F8] p-3">
                    <div class="text-center text-[12px] text-gray-600 mb-1">Work Progress Overview (Project:
                        {{ $chartTitle }})</div>
                    ...
                    <script>
                        (function() {
                            ...
                            pathPts.forEach((q, i) => {
                                ...
                                outer.setAttribute('cx', q.x);
                                outer.setAttribute('cy', q.y);
                                outer.setAttribute('r', '6');
                                outer.setAttribute('fill', '#fff');
                                outer.setAttribute('stroke', '#FF2D2D');
                                outer.setAttribute('stroke-width', '2');
                                gArea.appendChild(outer);

                                const inner = document.createElementNS(NS, 'circle');
                                inner.setAttribute('cx', q.x);
                                inner.setAttribute('cy', q.y);
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                ...
                <div class="rounded-2xl card-maroon border p-4 min-h-[140px] md:col-span-6">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <form method="GET" action="{{ route('dig.dashboard') }}" class="flex items-center gap-2">
                            ...
                            <input type="month" name="month_start" value="{{ $monthStartVal }}"
                                class="h-8 rounded-lg border border-red-300 bg-white/10 px-2 text-xs">
                            <span class="text-xs text-white/80">s/d</span>
                            <input type="month" name="month_end" value="{{ $monthEndVal }}"
                                class="h-8 rounded-lg border border-red-300 bg-white/10 px-2 text-xs">
                            <button
                                class="h-8 px-3 rounded-lg bg-white text-[#7A1C1C] text-xs font-semibold">Terapkan</button>
                        </form>
                    </div>
                    <div class="flex items-center gap-4">
                        <svg width="{{ $size }}" height="{{ $size }}"
                            ...
                            <circle cx="{{ $size / 2 }}" cy="{{ $size / 2 }}" r="{{ $r }}"
                                stroke="#A05353" stroke-width="{{ $stroke }}" fill="none"
                                opacity=".35" />
                            <circle cx="{{ $size / 2 }}" cy="{{ $size / 2 }}" r="{{ $r }}"
                            ...
                            <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-size="18"
                                font-weight="700" fill="#fff">{{ $acc }}%</text>
                        </svg>
                        <div class="text-xs text-white min-w-0 break-words">
                            <div>Periode: <span class="font-semibold">{{ $ws }}–{{ $we }}</span>
                            </div>
                            <div>Scope aktif: <span
                                    class="font-semibold">{{ $scope === 'mine' ? 'Tugas saya' : 'Semua' }}</span>
                            </div>
                            <div class="mt-1">Total project (scope global): <span
                                    class="font-semibold">{{ $totalScopeProjects }}</span></div>
                            <div class="mt-1">Selesai (periode): <span class="font-semibold">Saya
                                    {{ $completedMine }} • Semua {{ $completedAll }}</span></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-2xl card-maroon border p-4 grid min-h-[140px] md:col-span-3">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <form method="GET" action="{{ route('dig.dashboard') }}" class="flex items-center gap-2">
                            ...
                            <input type="hidden" name="scope" value="{{ $scope }}">
                            <input type="hidden" name="scope_not" value="{{ $scopeNot }}">
                            <select name="scope_meet" class="h-8 rounded-lg border bg-white/10 px-2 text-xs"
                                onchange="this.form.submit()">
                                <option value="all" {{ $scopeMeet === 'all' ? 'selected' : '' }}>Semua</option>
                                <option value="mine" {{ $scopeMeet === 'mine' ? 'selected' : '' }}>Tugas saya
                                </option>
                            </select>
                        </form>
                    </div>
                    <div class="text-4xl font-bold place-self-center text-white">{{ $meetDisplay }}</div>
                    <div class="text-[11px] text-white/80 mt-2 text-center">Saya: {{ $meetMine }} · Semua:
                        {{ $meetAll }}</div>
                </div>
                ...
                <div class="rounded-2xl card-maroon border p-4 grid min-h-[140px] md:col-span-3">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <form method="GET" action="{{ route('dig.dashboard') }}" class="flex items-center gap-2">
                            ...
                            <input type="hidden" name="scope" value="{{ $scope }}">
                            <input type="hidden" name="scope_meet" value="{{ $scopeMeet }}">
                            <select name="scope_not" class="h-8 rounded-lg border bg-white/10 px-2 text-xs"
                                onchange="this.form.submit()">
                                <option value="all" {{ $scopeNot === 'all' ? 'selected' : '' }}>Semua</option>
                                <option value="mine" {{ $scopeNot === 'mine' ? 'selected' : '' }}>Tugas saya
                                </option>
                            </select>
                        </form>
                    </div>
                    <div class="text-4xl font-bold place-self-center text-white">{{ $notMeetDisplay }}</div>
                    <div class="text-[11px] text-white/80 mt-2 text-center">Saya: {{ $notMine }} · Semua:
                        {{ $notAll }}</div>
                </div>
            </div>
        </section>
                        ...
                        @php
                            ...
                            $canDecideCompletion =
                                ...
                                (function () use ($project) {
                                    $all = $project->progresses->every(function ($pr) {
                                        $last =
                                            $pr->updates->first() ?: $pr->updates->sortByDesc('update_date')->first();
                            ...
                            foreach ($project->progresses as $p2) {
                                ...
                                $end2 = $p2->end_date
                                    ? \Illuminate\Support\Carbon::parse($p2->end_date)->startOfDay()
                                    : null;
                            ...
                            if ($hasOverdueUnmet) {
                                $othersAllConfirmed = $project->progresses->every(function ($p3) {
                                    ...
                                    $end3 = $p3->end_date
                                        ? \Illuminate\Support\Carbon::parse($p3->end_date)->startOfDay()
                                        : null;
                                ...
                                if (is_null($project->meets_requirement) && $othersAllConfirmed) {
                                    $canDecideCompletion = true;
                                }
                            }
                                ...
                                <div class="flex flex-col gap-3 w-full">
                                    ...
                                    <div class="w-full">
                                        <div class="flex items-center justify-between mb-1">
                                            ...
                                            <span
                                                class="text-[11px] text-gray-700 font-semibold">{{ $sumReal }}%</span>
                                        </div>
                            ...
                            <div id="progressForm-{{ $project->id }}"
                                ...
                                <form method="POST" action="{{ route('projects.progresses.store', $project->id) }}"
                                    ...
                                    <input name="name" required placeholder="Nama Progress"
                                        class="rounded-xl bg-white border border-[#C89898] px-3 py-2 outline-none md:col-span-2">
                                ...
                                <div class="scroll-thin grid md:grid-cols-2 gap-4 max-h-[280px] overflow-y-auto pr-1">
                                    @forelse($project->progresses as $pr)
                                        @php
                                            ...
                                            $roleC = $creator?->role;
                                            $ownerRoleLabel =
                                                $roleC === 'digital_banking' ? 'DIG' : ($roleC === 'it' ? 'IT' : '—');
                                        @endphp
                                        ...
                                        <div class="rounded-2xl bg-[#F7E4E4] p-4 border border-[#E7C9C9]">
                                            ...
                                            @if ($isUnmet)
                                                <div
                                                    class="mb-2 text[12px] rounded-lg border border-red-300 bg-red-50 text-red-700 px-3 py-2">
                                                    Melewati timeline selesai, realisasi belum mencapai target & belum
                                                    dikonfirmasi.
                                                </div>
                                            @endif
                                            ...
                                            <div class="mt-2 text-sm">
                                                <div class="grid grid-cols-[auto,1fr] gap-x-4 gap-y-1">
                                                    ...
                                                    <span>:
                                                        {{ $pr->start_date ? \Illuminate\Support\Carbon::parse($pr->start_date)->timezone('Asia/Jakarta')->format('d M Y') : '-' }}</span>
                                                    <span>Timeline Selesai</span>
                                                    <span>:
                                                        {{ $pr->end_date ? \Illuminate\Support\Carbon::parse($pr->end_date)->timezone('Asia/Jakarta')->format('d M Y') : '-' }}</span>
                                                    <span>Target Progress</span><span>:
                                                        {{ (int) $pr->desired_percent }}%</span>
                                            ...
                                            <div id="editProgress-{{ $pr->id }}" class="hidden mb-3">
                                                <form method="POST"
                                                    ...
                                                    @csrf @method('PUT')
                                                    <input name="name" value="{{ old('name', $pr->name) }}"
                                                        required
    ...
    @unless ($isOwner) disabled @endunless>
                                            ...
                                            <div class="mt-3">
                                                ...
                                                @can('createUpdate', $pr)
                                                    ...
                                                    <form method="POST"
                                                        ...
                                                        <input type="number" name="percent" min="0"
                                                            max="100" placeholder="%"
                                            ...
                                            <div class="mt-3">
                                                <div class="mt-2">
                                                    @if (!$alreadyConfirmed)
                                                        <form method="POST"
                                                            action="{{ route('progresses.confirm', $pr->id) }}">
                                                            @csrf
                                                            <button
                                            ...
                                            @unless ($isOwner)
                                                <p class="mt-2 text-xs text-gray-600">
                                                    *Progress ini dibuat oleh <strong>{{ $ownerRoleLabel }}</strong> —
                                                    {{ $creator?->name ?? '—' }}. Anda hanya dapat melihat tanpa mengubah.
                                                </p>
                                            @endunless
                                        </div>
                                    @empty
