...
@section('content')
    ...
    <div class="{{ $container }} py-3 flex items-center justify-between">
        <div class="flex items-center gap-3">
            @if ($totalUnread > 0)
                <span
                    class="inline-flex items-center justify-center min-w-[1.5rem] h-6 rounded-full bg-[#7A1C1C] text-white text-xs px-2">
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        @forelse($todayCollection as $n)
                            ...
                            <div
                                class="rounded-xl px-5 py-4 border {{ $isUnread ? 'border-[#7A1C1C] bg-[#F2DCDC]' : 'border-[#E7C9C9] bg-white' }}">
                                <div class="flex items-start justify-between gap-4">
                                    <div class="min-w-0">
                                        @if ($type === 'dig_project_created')
                                            <div class="text-[15px] font-semibold">Project Baru Dibuat</div>
                                            <div class="mt-1 text-sm">
                                                <div><span class="font-semibold">Nama Project</span>: {{ $pName }}
                                                </div>
                                                <div class="mt-1"><span class="font-semibold">Tanggal</span>:
                                                    {{ $dateText }} • {{ $timeText }} WIB</div>
                                                @if ($message)
                                        ...
                                        @elseif($type === 'dig_completion_decision')
                                            ...
                                            <div class="mt-1 text-sm">
                                                ...
                                                <div class="mt-1">
                                                    ...
                                                    <span
                                                        class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold {{ $isMeet ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                                        {{ $statusLabel }}
                                                    </span>
                                                </div>
                                                <div class="mt-1"><span class="font-semibold">Tanggal</span>:
                                        ...
                                        @else
                                            ...
                                            <div class="mt-1 text-sm">
                                                ...
                                                <div class="mt-1 text-xs text-gray-600">{{ $dateText }} •
                                                    {{ $timeText }} WIB</div>
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    <div class="text-right shrink-0">
                                        ...
                                        <div class="mt-2 flex items-center gap-2 justify-end">
                                            ...
                                            @if ($isUnread)
                                                <form method="POST"
                                                    action="{{ route('it.notifications.read', $n->id) }}">
                                                    @csrf
                                                    <button class="text-xs underline text-[#7A1C1C]">Tandai
                                                        terbaca</button>
                                                </form>
                                            @endif
                                        </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-3">
                                @foreach ($items as $n)
                                    ...
                                    <div
                                        class="rounded-xl px-5 py-4 border {{ $isUnread ? 'border-[#7A1C1C] bg-[#FDF3F3]' : 'border-[#E7C9C9] bg-white' }}">
                                        <div class="flex items-start justify-between gap-4">
                                            <div class="min-w-0">
                                                @if ($type === 'dig_project_created')
                                                    <div class="text-[15px] font-semibold">Project Baru Dibuat</div>
                                                    <div class="mt-1 text-sm">
                                                        <div><span class="font-semibold">Nama Project</span>:
                                                            {{ $pName }}</div>
                                                        @if ($message)
                                                            <div class="text-gray-700 mt-1">{{ $message }}</div>
                                                        @endif
                                                ...
                                                @elseif($type === 'dig_completion_decision')
                                                    @php
                                                        ...
                                                        $isMeet = $decision === 'memenuhi';
                                                    @endphp
                                                    <div class="text-[15px] font-semibold">Keputusan Penyelesaian Project
                                                    </div>
                                                    <div class="mt-1 text-sm">
                                                        <div><span class="font-semibold">Nama Project</span>:
                                                            {{ $pName }}</div>
                                                ...
                                                @else
                                                    <div class="text-[15px] font-semibold">Notifikasi</div>
                                                    <div class="mt-1 text-sm">
                                                        <div class="text-gray-700">{{ $message ?: 'Ada pembaruan.' }}
                                                        </div>
                                                    </div>
                                                @endif
                                            </div>
