...
@section('content')
    ...
    @php
        ...
        $mine = $all->filter(
            fn($p) => (int) ($p->developer_id ?? 0) === $userId || (int) ($p->created_by ?? 0) === $userId,
        );
            ...
            <div class="rounded-2xl card-maroon border p-4 min-h-[140px] md:col-span-6">
                ...
                <div class="flex items-center gap-4">
                    <svg width="{{ $size }}" height="{{ $size }}"
                        ...
                        <circle cx="{{ $size / 2 }}" cy="{{ $size / 2 }}" r="{{ $r }}" stroke="#fff"
            ...
            <div class="rounded-2xl card-maroon border p-4 grid min-h-[140px] md:col-span-3">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <form method="GET" action="{{ route('it.dashboard') }}" class="flex items-center gap-2">
                        ...
                        <input type="hidden" name="scope" value="{{ $scope }}">
                        <input type="hidden" name="scope_not" value="{{ $scopeNot }}">
                        <select name="scope_meet"
                            class="h-8 rounded-lg border border-white/70 bg-white px-2 text-xs text-gray-900"
                        ...
                        <div class="grid md:grid-cols-[1fr,auto] items-start gap-4">
                            <div class="flex flex-col gap-3 w-full">
                                ...
                                <div class="w-full">
                                    ...
                                    <div class="mt-1 text-[11px] text-gray-600">Target (akumulasi "Keinginan Awal"):
                                        {{ $sumDesired }}%</div>
                                </div>
                            ...
                            <div class="flex items-start gap-2 justify-end">
                                @can('finalize', $project)
                                    @php
                                        $canDecideCompletion =
                                            $project->progresses->every(function ($pr) {
                                                $last = $pr->updates->sortByDesc('update_date')->first();
                                                $real = $last
                                    ...
                                    @if ($canDecideCompletion)
                                        <form method="POST"
                                            action="{{ route('projects.setCompletion', $project->id) }}" class="mr-2">
                                            @csrf @method('PATCH')
                                        ...
                                        <form method="POST"
                                            ...
                                            <button class="px-3 py-1.5 text-xs rounded-full bg-[#7A1C1C] text-white">Tidak
                                                Memenuhi</button>
                                        </form>
                                    @endif
                                @endcan
                                ...
                                @can('update', $project)
                                    <a href="{{ route('semua.projects.edit', $project->id) }}"
                                        class="p-2 rounded-lg bg-white hover:bg-[#FFF2F2] border" title="Edit Project">
                            ...
                            <div class="scroll-thin grid md:grid-cols-2 gap-4 max-h-[280px] overflow-y-auto pr-1">
                                @forelse($project->progresses as $pr)
                                    @php
                                        ...
                                        $isUnmet =
                                            $isOverdue && !$alreadyConfirmed && $realisasi < (int) $pr->desired_percent;
                                    @endphp
                                    ...
                                    <div class="rounded-2xl bg-[#F7E4E4] p-4 border border-[#E7C9C9]">
                                        ...
                                        <div class="mt-3">
                                            ...
                                            @if (!$alreadyConfirmed)
                                                @can('confirm', $pr)
                                                    ...
                                                    @endphp
                                                    <form method="POST"
                                                        action="{{ route('progresses.confirm', $pr->id) }}" class="mt-2">
                                                        @csrf
                                                        <button
                                                ...
                                                @else
                                                    <div class="mt-2">
                                                        <button
                                                            class="rounded-xl bg-gray-300 text-white px-4 py-2 text-sm font-semibold cursor-not-allowed"
                                            ...
                                            @else
                                                <span
                                                    class="inline-flex items-center rounded-full bg-green-100 text-green-700 px-3 py-1 text-xs font-semibold mt-2">
                                                    Sudah dikonfirmasi
                                                </span>
                                            @endif
                                        ...
                                        @cannot('update', $pr)
                                            <p class="mt-2 text-xs text-gray-600">
                                                *Progress ini dibuat oleh pengguna lain. Anda hanya dapat melihat tanpa
                                                mengubah.
                                            </p>
                                        @endcannot
                                    </div>
